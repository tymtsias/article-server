...
class DoobieUserRepo(transactor: Aux[IO, Unit]) extends UserRepo {
  ...
  override def save(user: UserWithEncryptedPassword): Future[Int] = saveQuery(user).run.transact(transactor).unsafeToFuture()

  override def getHash(email: String): Future[String] = sql"""select "password" from users where email = $email""".query[String].unique.transact(transactor).unsafeToFuture()
