...
class DoobieFavoritesRepo(transactor: Aux[IO, Unit]) extends FavoritesRepo{
  ...
  def increaseFavoriteCount(slug: String) = sql"update article set favorites_count = favorites_count + 1 where slug = $slug;".update
  override def favorite(slug: String, email: String)(implicit ec: ExecutionContext): Future[Unit] = favoriteQuery(slug, email).run.flatMap {
    case 0 => 0.pure[ConnectionIO]
    case _ => increaseFavoriteCount(slug).run
  }.transact(transactor).unsafeToFuture()(catsGlobal).map(_ => ())
  ...
  def decreaseFavoriteCount(slug: String) = sql"update article set favorites_count = favorites_count - 1 where slug = $slug;".update
  override def unfavorite(slug: String, email: String)(implicit ec: ExecutionContext): Future[Unit] = unfavoriteQuery(slug, email).run.flatMap{
    case 0 => 0.pure[ConnectionIO]
